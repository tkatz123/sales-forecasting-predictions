...
#List of packages that are required to be installed and loaded
required_packages <- c("tidyverse", "lubridate", "ggplot2", "skimr", "plotly")

#Retrieves list of packages that are installed
installed_packages <- rownames(installed.packages())

#Checks to see if requires packages are installed and libraries them, if not downloads them
...
df <- read_csv('../Data/preprocessed_data.csv')
...
To gain an initial understanding of the cleaned dataset, we used the `skimr::skim()` function to generate a structured summary of the data. The dataset consists of 1,017,209 observations and 25 variables, spanning sales transactions across multiple Rossmann store locations.
...
*Numeric Variables*
  ...
  -All numeric variables have no null values 
